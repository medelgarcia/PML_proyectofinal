How to predict the manner on doing an exercise
========================================================

### Motivation:
With the use and cheap of news technologies has been appeared many new data available. In this study I am using data from four acelerometer located on: belt, forearm, arm and dumbbell while an exercise is performed. As goal of this analysis is to predict using the input variables if the exercise is performed correctly or mistakenly; in order to build a pattern to predict there is available two datasets, one for training and other one for testing.
The imput variables are the acceleration of each 3D accelerometer for each of the four accelerometer.
There are six participant and each one will execute the exercice wrong in five ways and correct.
This data comes from source: <http://groupware.les.inf.puc-rio.br/har>
...
The assignment was done by Juan M. Medel Garcia at 27th of July in 2014.
